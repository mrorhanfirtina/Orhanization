...
namespace Monstersoft.VennWms.Main.Application.Features.SerialFeatures.SerialLogs.Commands.Create;

public class CreateSerialLogCommand : IRequest<CreatedSerialLogResponse>, ITransactionalRequest, ICacheRemoveRequest, ILoggableRequest, ILocalityRequest, ISecuredRequest
{
    ...
    public CreateSerialLogDto SerialLog { get; set; }
    public SerialLogsDetailLevel DetailLevel { get; set; }

    public class CreateSerialLogCommandHandler : IRequestHandler<CreateSerialLogCommand, CreatedSerialLogResponse>
    {
        ...
        public CreateSerialLogCommandHandler(ISerialLogRepository serialLogRepository, IMapper mapper, SerialLogBusinessRules serialLogBusinessRules)
        {
        ...
        public async Task<CreatedSerialLogResponse> Handle(CreateSerialLogCommand request, CancellationToken cancellationToken)
        {
            _serialLogBusinessRules.CreateRequest()
                .CheckDepositorCompany(request.UserRequestInfo.RequestUserLocalityId);


            SerialLog serialLog = _mapper.Map<SerialLog>(request.SerialLog);
            serialLog.Id = Guid.NewGuid();
            serialLog.CreatedDate = DateTime.Now;

            await _serialLogRepository.AddAsync(serialLog);
            ...
            if (request.DetailLevel.AnyPropertyTrue())
            {
                ...
                return _mapper.Map<CreatedSerialLogResponse>(response);
            }
            else
            {
                var response = await _serialLogRepository.GetAsync(predicate: x => x.Id == serialLog.Id,
