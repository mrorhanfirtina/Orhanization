...
public class CustomerConfiguration : IEntityTypeConfiguration<Customer>
{
    public void Configure(EntityTypeBuilder<Customer> builder)
    {
        ...
        builder.ToTable("Customers", "depositor").HasKey(p => p.Id);
        #endregion
        ...
        builder.HasIndex(p => new { p.Code, p.Name, p.CompanyId, p.AddressId,p.DepositorCompanyId, p.CreatedDate }, name: "IX_Customers_Areas");
        #endregion

        #region İlişki Tanımları
        builder.HasOne(p => p.Address).WithOne().HasForeignKey<Customer>(p => p.AddressId).OnDelete(DeleteBehavior.Restrict);
